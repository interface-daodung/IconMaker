...
def test_missing_source_raises_filenotfound(tmp_path):
    with pytest.raises(FileNotFoundError):
        converter.convert_png_to_ico(tmp_path / "nope.png", tmp_path / "a.ico")


def test_broken_image_raises_valueerror(broken_file, tmp_path):
    with pytest.raises(ValueError):
        converter.convert_png_to_ico(broken_file, tmp_path / "a.ico")


def test_text_file_with_png_extension_rejected(fake_png_nonimage, tmp_path):
    with pytest.raises(ValueError):
        converter.convert_png_to_ico(fake_png_nonimage, tmp_path / "a.ico")


def test_non_png_format_rejected(jpeg_file, tmp_path):
    with pytest.raises(ValueError):
        converter.convert_png_to_ico(jpeg_file, tmp_path / "a.ico")
...
def test_rgb_input_produces_valid_rgba_icon(png_rgb, tmp_path):
    ...
    out = converter.convert_png_to_ico(png_rgb, dest, sizes=[32])
...
def test_rgba_alpha_is_preserved_in_smallest_frame(png_rgba, tmp_path):
    ...
    dest = tmp_path / "a.ico"
    converter.convert_png_to_ico(png_rgba, dest, sizes=[16])
...
def test_solid_rgba_keeps_mostly_opaque(png_rgba, tmp_path):
    dest = tmp_path / "a.ico"
    converter.convert_png_to_ico(png_rgba, dest, sizes=[32])
    frame = ico_get_frame(dest, (32, 32))
...
def test_default_sizes_all_present_in_output(png_rgb, tmp_path):
    dest = tmp_path / "a.ico"
    converter.convert_png_to_ico(png_rgb, dest)
...
def test_small_source_is_upscaled_to_requested_size(png_small, tmp_path):
    dest = tmp_path / "a.ico"
    converter.convert_png_to_ico(png_small, dest, sizes=[256])
...
def test_large_image_converts_without_error(png_large, tmp_path):
    ...
    converter.convert_png_to_ico(png_large, dest, sizes=[256, 32])
    assert ico_frame_sizes(dest) == {(32, 32), (256, 256)}


def test_duplicate_and_unsorted_sizes_deduplicated(png_rgb, tmp_path):
    dest = tmp_path / "a.ico"
    converter.convert_png_to_ico(png_rgb, dest, sizes=[48, 32, 48])
...
def test_creates_missing_parent_directories(png_rgb, tmp_path):
    ...
    converter.convert_png_to_ico(png_rgb, dest, sizes=[16])
    assert dest.is_file()


def test_destination_directory_is_rejected(png_rgb, tmp_path):
    with pytest.raises(ValueError):
        converter.convert_png_to_ico(png_rgb, tmp_path)
...
def test_output_is_readable_ico_with_requested_sizes(png_rgba, tmp_path):
    dest = tmp_path / "a.ico"
    converter.convert_png_to_ico(png_rgba, dest, sizes=[16, 24])
