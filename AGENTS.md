...
- **Tên:** IconMaker
- **Mục đích:** Xử lý ảnh/icon bằng Pillow (convert PNG→ICO, bo góc, tách sprite + OCR, build ICO chất lượng cao, đặt icon thư mục qua desktop.ini), giao diện **CustomTkinter** tab và **Launcher C#** chạy trên Windows.
- **Stack:** Python 3.13, Pillow, CustomTkinter, .NET / C# (launcher).
- **Kiến trúc (plan 008):** `main.py` (entry) + `core/` (logic thuần, cấm import GUI) + `gui/` (chỉ hiển thị: main_window, base_tool, widgets, theme) + `tools/` (mỗi tool 1 tab + hàm `run_*` gọi core, đăng ký ở `registry.py`) + `assets/`. Plan + trạng thái trong `.agents/README.md`.
...
| `main.py` | Entry point GUI (`pythonw main.py`) |
| `core/` | Logic thuần, không phụ thuộc GUI (convert, image_ops, sprites, icons, foldericon, ...) |
| `gui/` | Chỉ hiển thị (main_window, base_tool, widgets, theme) |
| `tools/` | Mỗi tool 1 tab + hàm `run_*`; đăng ký ở `registry.py` |
...
10. **GUI chạy bằng pythonw, không hiện console.** Mở GUI trực tiếp bằng `make gui` (`pythonw -m iconmaker.gui`); launcher C# ưu tiên `pythonw`/`pyw` và đặt `CreateNoWindow=true`.
...
12. **GUI chỉ là giao diện.** Mọi logic nằm trong `core/` (dùng chung, cấm import GUI) hoặc `tools/` (hàm `run_*`/`parse_*` cấp module của từng tab). Tab mới = thêm module trong `tools/` + 1 dòng trong `tools/registry.py`.
13. **Launcher là template tray-clone, không phải phần của app.** `launcher/` theo khung `TrayDemo`: sinh exe C# gán icon + nối tới folder server; chỉ sửa `AppConfig.cs` + `Assets/icon.ico` khi clone, csproj gen bằng `dotnet new winforms` rồi patch 2 dòng icon (không copy tay); app mới sinh bằng `launcher/new-launcher.ps1` (`make new-launcher NAME=...`); `Port=0` + `ProjectDir=""` nghĩa là app GUI/tự dò root.

## Quy ước code

- Python: mã nguồn `core/` + `gui/` + `tools/` + `main.py` ở gốc project, test trong `tests/` (dùng pytest).
