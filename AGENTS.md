...
19. **Tool Build Launcher build exe tray từ 1 thư mục server.** Tab "Build Launcher": ô thư mục server (dialog mở sẵn `C:\Users\inter\Project`), ô chọn 1 file ICO từ dự án (prefill ICO mới nhất `output/icons`), ô nhập tên launcher (bỏ trống = lấy tên thư mục server). Chạy build: copy icon vào thư viện `~/OneDrive/Pictures/Icon` đặt tên `<ten-launcher>.ico` qua `service/launcher.py::install_build_icon` (Luật 8) rồi gọi `launcher/build-launcher.ps1 -ServerDir -Name -Icon -OutDir -Mode`; `.exe` ra `output/launchers/` (`OUTPUT_LAUNCHER` trong `core/paths.py`). Contract trong `service/launcher.py` (`sanitize_launcher_name`, `project_clean_name`, `validate_server_dir`, `build_command`, `run_build`); CLI `python -m service.launcher <server> [--icon] [--name] [--out] [--mode]`, target `make build-launcher SERVER=... ICON=... LNAME=... [MODE=...]`.
