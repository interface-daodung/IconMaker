...
## 2026-09-16 (app GUI thuần — bỏ CLI)

- **[D30] Bỏ toàn bộ CLI/pipeline, app chỉ còn GUI chạy bằng shortcut `pythonw`:** Makefile rút còn 2 target `run` (mở GUI bằng `pythonw src/main.py`) và `cls` (xóa `__pycache__` + `.pytest_cache`); xóa `main()`/`__main__` khỏi mọi module `service/` (convert, export, format_convert, image_ops, resize, sprites, icons, foldericon, junction, launcher) cùng các hàm/hằng chỉ CLI dùng (`convert_png_to_ico`, `convert_format_file`, `target_formats`, `first_image`, `resolve_output_path`, `with_extension`, `OUTPUT_CONVERT`, `OUTPUT_CONVERT_FORMAT`, `USAGE`, `FORMAT_NAMES` bị thu gọn); `src/main.py` tự thêm `src/` vào `sys.path` + neo cwd về gốc project nên shortcut Windows `pythonw "…\src\main.py"` chạy được từ bất kỳ thư mục nào. Test lỗi thời (CLI `main`) được xóa, thay bằng test hợp đồng thật (`convert_image_to_ico`, `compress_bytes`, `_bootstrap`, default output dirs).
  → Luật 7, 10, 14 (`AGENTS.md`); `Makefile`, `src/main.py`, `src/core/paths.py`, `src/core/file_utils.py`.